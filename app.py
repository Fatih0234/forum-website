...
class Reply(db.Model):
    ...
    # thread = db.relationship('Thread', backref=db.backref('replies', lazy=True))
...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    # we need to pass the threads to the template
...
def thread(thread_id):
    ...
    if form.validate_on_submit():
        new_reply = Reply(user_id=current_user.id, message=form.message.data, date_created=datetime.now())
